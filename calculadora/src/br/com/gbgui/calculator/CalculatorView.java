...
public class CalculatorView extends AbstractClass implements InterfaceClass{
	...
	private static List<String> optionsCalculator = Arrays.asList("Soma", "Subtração", "Multiplicação", "Divisão",
			"Potenciação");
	...
	public void calculate(Double firstValue, Double secondValue, int option) {
		if(option == 1) {
			...
			Double result = firstValue - secondValue;
			this.printResult("Resultado da subtração: ", result);
		}else if(option == 3) {
			Double result = firstValue * secondValue;
			this.printResult("Resultado da multiplicação: ", result);
		}else if(option == 4) {
			Double result = firstValue / secondValue;
			this.printResult("Resultado da divisão: ", result);
		}else if(option == 5) {
			Double result = Math.pow(firstValue, secondValue);
			this.printResult("Resultado da potenciação: ", result);
		}
		this.isContinue();
	}
